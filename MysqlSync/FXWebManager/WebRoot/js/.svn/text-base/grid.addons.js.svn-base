			...
			var formFill = function(){
				...
				$.each(self.p.filterModel,function(i,n){
					...
					if(self.p.formtype=='horizontal'){
						...
						$(tr).append(td);
					} else if(self.p.formtype=='vertical'){
						tr1 = document.createElement("tr");
						$(tr1).append(tl).append(td);
						$(tbl).append(tr1);
					} else {
						var formtypeDetail = self.p.formtypeDetail;
						var count = 0;
						if(formtypeDetail.length > 0){
							for(var m=0;m<formtypeDetail.length;m++){
								count = count + formtypeDetail[m];
								if(i+1 <= count){
									if(i == count-formtypeDetail[m]){
										tr1 = document.createElement("tr");
									}
									$(tr1).append(tl).append(td);
									$(tbl).append(tr1);
