...
public class BaseReciver implements MessageListener {

	private static Log logger = LogFactory.getLog(BaseReciver.class);
	...
	// 自动 true 手动接收 false;
	public BaseReciver(boolean autoflag, String ip, int port, boolean syncflag,
			String name) {
		this.ip = ip;
		this.port = port;
	...
	public boolean start(IReciveHandler handler) {
		...
		try {
			if (autoflag && getConsumer() != null) {
				...
				logger.info("侦听通道 " + name + "启动  iP: " + ip);
				b = true;
			}
		} catch (Exception e) {
			e.printStackTrace();
...
//				log.info("recive data");
			...
			logger.error("recive data error : name: "+name, e);
			b = false;
		}
		return b;
	}
...
//		Mail msg = createMsg(message);
		...
		try {
			if (b) {
				...
				logger.error("业务通道 " + name + " 处理异常");
				session.recover();
			}
		} catch (JMSException e) {
			e.printStackTrace();
			logger.error("业务通道 " + name + " 处理异常",e);
		}
	}
