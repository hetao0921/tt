...
public class DataVersionDao {
	...
	public void insert(DataVersion dataVersion) {
		...
		try {
			NVMPHibernate.getNVMPhibernate().save(dataVersion);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	public List<HashMap<String, String>> getExistSource(String centerid,int version) {
		NVMPHibernate db = NVMPHibernate.getNVMPhibernate();
		String hql="select new Map(centerid as centerid,centername as centername,centerip as centerip,version as version) from DataVersion where centerid='"
				+ centerid + "' and version='"+String.valueOf(version)+"'";
		return db
				.executeQuerySql(hql);
	}
