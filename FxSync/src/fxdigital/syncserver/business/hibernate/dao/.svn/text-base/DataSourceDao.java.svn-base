...
public class DataSourceDao {
	...
	public void insert(DataSource dataSource) {
		...
		try {
			NVMPHibernate.getNVMPhibernate().save(dataSource);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int updateSource(String centerID,int version,String fileaddress){
		...
		NVMPHibernate.getNVMPhibernate().deleteOrUpdate(sql, new Object[]{fileaddress,centerID,version});
		 return 1;
	}
