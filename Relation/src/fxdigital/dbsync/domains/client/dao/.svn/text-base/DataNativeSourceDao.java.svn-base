...
//        String hql=	"from DataNativeSource as a right join select max(id) id from DataNativeSource group by centerid as b on b.id = a.id where a.id is not null ORDER BY centerid";	
	    
        
//        String hql="select new Map(max(id),centerid,centername,centerip,version,data.update,uuid,downstartdate,downenddate,flag) from DataNativeSource data where data.id is not null ORDER BY data.centerid";
        String hql="from DataNativeSource as a where a.id in (select max(id) from DataNativeSource group by centerid) and a.id is not null ORDER BY centerid";
