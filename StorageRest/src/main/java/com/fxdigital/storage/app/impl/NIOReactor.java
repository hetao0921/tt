...
import com.fxdigital.storage.app.IEventHandler;
import com.fxdigital.storage.app.IReactor;
...
class Timer implements ITimer, Comparable, Comparator {
	...
	public int compare(Object o1, Object o2) {
		...
		if (timer1.expireTime > timer2.expireTime) {
			return 1;
		} else if(timer1.expireTime == timer2.expireTime) {
			return 0;
		} else {
			return -1;
		}
...
public class NIOReactor implements IReactor, Runnable {
	...
	void select() throws IOException {
		if (timerQueue.size() > 0) {
			...
			long timeout = (timer.getExpireTime() - currentTime)
					/ (1000L * 1000L);
	...
	private void checkIo() {
		...
		while (iter.hasNext()) {
			...
			try {
				attachment.getEventHandler().handleIO(key,
						attachment.getUserContext());
			} catch (Exception e) {
				logger.info(e);
			}
			checkTimer();
	...
	private void unregisterImpl(SelectableChannel channel, int ops) {
		...
		try {
			...
			if (attachment != null && attachment.getEventHandler() != null)
				attachment.getEventHandler().handleError(e,
						attachment.getUserContext());
		}
	}
	...
	private void registerImpl(SelectableChannel channel, int ops,
			IEventHandler handler, Object userContext) {
		try {
	...
	public void register(SelectableChannel channel, int ops,
		...
		if (isOwnered()) {
			...
			class RegisterAction implements Runnable {
				...
				public RegisterAction(SelectableChannel _channel, int _ops,
						IEventHandler _handler, Object _userContext) {
					this.channel = _channel;
					this.ops = _ops;
					this.handler = _handler;
			...
			this.runInReactor(new RegisterAction(channel, ops, handler,
					userContext));
		}
	}
